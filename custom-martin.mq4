...
//--- 
        // SymbolInfoTick(_Symbol,currentTick);
         ...
         if(currentCandle.Date != lastCandle.Date)
         {
            ...
            if(tradeType != 0 &&  getOpenedOrderNo() == 0 && martinIndex == 0)
            {
               openTrade(tradeType);
            }
            ...
            if(lastTradeDir != 0 &&  getOpenedOrderNo() == 0)
            {
               ...
               if(martinIndex > 0)
               {
                  ...
                  Print("order opened for martin recover , dir : ",lastTradeDir);
                  openTrade(lastTradeDir);
               }
